...
import {
  ...
  fetchSubject,
  searchOpenLibrary,
  searchByGenre as olSearchByGenre,
  searchByAuthor as olSearchByAuthor,
  type OLBook,
} from '../lib/openLibrary';
import {
  searchByGenre as gbSearchByGenre,
  searchByAuthor as gbSearchByAuthor,
  type GoogleBookResult,
} from '../lib/googleBooks';

type DiscoveryTab = 'trending' | 'genres' | 'foryou';
...
const GENRES = [
  ...
  'Romance', 'Thriller', 'History', 'Biography',
  'Science', 'Philosophy',
];
...
export default function Recommendations({ onAddBook }: { onAddBook: (book: Partial<Book>) => void }) {
  ...
  const [trendingBooks, setTrendingBooks] = useState<OLBook[]>([]);
  const [trendingLoading, setTrendingLoading] = useState(false);
  const [trendingError, setTrendingError] = useState(false);
  const [selectedGenre, setSelectedGenre] = useState<string>('Fiction');
  const [genreBooks, setGenreBooks] = useState<OLBook[]>([]);
  ...
  useEffect(() => {
    getBooks({ status: 'finished' }).then(books => {
      ...
      for (const b of books) { if (b.genre) genreCounts[b.genre] = (genreCounts[b.genre] ?? 0) + 1; }
      ...
      const topAuthors = Object.entries(authorRatings).map(([author, { total, count }]) => ({ author, avgRating: Math.round((total / count) * 10) / 10 })).filter(a => a.avgRating >= 4).sort((a, b) => b.avgRating - a.avgRating).slice(0, 3);
      const withPages = books.filter(b => b.pages != null && b.pages > 0);
      const avgPages = withPages.length ? Math.round(withPages.reduce((s, b) => s + (b.pages ?? 0), 0) / withPages.length) : 300;
      const lengthBucket: 'short' | 'medium' | 'long' = avgPages < 250 ? 'short' : avgPages > 400 ? 'long' : 'medium';
  ...
  const buildSections = useCallback((prefs: Preferences) => {
    ...
    if (prefs.topGenres.length) {
      ...
      newSections.push({ title: `Because you love ${primaryGenre}`, books: [], loading: true });
      newSections.push({ title: `Highly rated in your genres`, books: [], loading: true });
  ...
  useEffect(() => {
    if (preferences) {
      if (!isOnline) { setOfflineError(true); setSections([]); return; }
  ...
  useEffect(() => {
    ...
    fetchTrending(20)
      .then(data => { setTrendingBooks(data); setTrendingLoading(false); })
      .catch(() => { setTrendingError(true); setTrendingLoading(false); });
  }, [activeTab]);
  ...
  useEffect(() => {
    ...
    setGenreError(false);
    fetchSubject(selectedGenre, 24)
      .then(data => { setGenreBooks(data.works); setGenreLoading(false); })
      .catch(() => { setGenreError(true); setGenreLoading(false); });
  ...
  const handleSearch = useCallback((q: string) => {
    ...
    searchOpenLibrary(q, 8).then(data => { setSearchResults(data || []); }).catch(() => { setFetchError(true); }).finally(() => { setSearchLoading(false); });
  }, [isOnline]);
  ...
  function handleAddBook(book: Recommendation) {
    onAddBook({ title: book.title, author: book.author, pages: book.pages, genre: book.genre, cover_url: book.coverUrl, description: book.description, language: book.language, status: 'planned' });
  }

  function handleOLBookAdd(book: OLBook) {
    onAddBook({ title: book.title, author: book.author, pages: book.pages, genre: book.genre, cover_url: book.coverUrl ?? undefined, description: book.description ?? undefined, language: book.language ?? undefined, status: 'planned' });
  }

  const totalBooks = sections.reduce((s, sec) => s + sec.books.length, 0);
  const filteredSections = sections.map(sec => ({ ...sec, books: langFilter === 'all' ? sec.books : sec.books.filter(b => b.language === langFilter) })).filter(sec => sec.books.length > 0 || sec.loading);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (active && (active.tagName === 'INPUT' || active.tagName === 'TEXTAREA')) { (active as HTMLElement).blur(); }
    };
    window.addEventListener('touchstart', onTouchStart, { passive: true });
    window.addEventListener('touchend', onTouchEnd, { passive: true });
    window.addEventListener('scroll', handleScroll, { passive: true });
    return () => { window.removeEventListener('touchstart', onTouchStart); window.removeEventListener('touchend', onTouchEnd); window.removeEventListener('scroll', handleScroll); };
  }, []);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {(offlineError || fetchError) && (
        <div style={{ background: 'rgba(255,77,106,0.08)', border: '1px solid rgba(255,77,106,0.2)', borderRadius: 8, padding: '16px 20px', textAlign: 'center' }}>
          ...
          <button
            onClick={() => { if (!navigator.onLine) return; setOfflineError(false); setFetchError(false); if (preferences) buildSections(preferences); }}
            style={{ padding: '8px 20px', borderRadius: 6, background: 'rgba(0,209,255,0.1)', border: '1px solid rgba(0,209,255,0.3)', color: '#00d1ff', fontSize: 11, cursor: 'pointer', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.08em' }}
          >
      ...
      {activeTab === 'foryou' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          {searchResults.length > 0 && (() => {
            ...
            return filtered.length > 0 ? (<><SectionHeader title={`Search: "${searchQuery}"`} count={filtered.length} /><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', paddingBottom: 8 }}>{filtered.map(book => <BookCard key={book.googleId} book={book} onAdd={() => handleAddBook(book)} inWishlist={wishlist.includes(book.googleId)} onWishlist={() => addToWishlist(book.googleId)} />)}</div></>) : null;
          })()}
          {preferences && (
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
...
function OLBookCard({ book, onAdd }: { book: OLBook; onAdd: () => void }) {
...
function GenreSection({ genre, wishlist, onAdd, onWishlist }: { genre: string | undefined; wishlist: string[]; onAdd: (b: Recommendation) => void; onWishlist: (id: string) => void }) {
  const [books, setBooks] = useState<Recommendation[]>([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => { if (!genre) return; setLoading(true); gbSearchByGenre(genre, undefined, undefined, 8).then(data => { setBooks(data || []); setLoading(false); }).catch(() => setLoading(false)); }, [genre]);
  if (loading) return <div style={{ height: 200, background: 'rgba(255,255,255,0.03)', borderRadius: 6 }} />;
  if (!books.length) return null;
  return (<div><SectionHeader title={`${genre} — highly rated`} count={books.length} /><div style={{  display: 'flex', gap: 10, flexWrap: 'wrap', paddingBottom: 8 }}>{books.map(book => <BookCard key={book.googleId} book={book} onAdd={() => onAdd(book)} inWishlist={wishlist.includes(book.googleId)} onWishlist={() => onWishlist(book.googleId)} />)}</div></div>);
}

function AuthorSection({ author, wishlist, onAdd, onWishlist }: { author: string; wishlist: string[]; onAdd: (b: Recommendation) => void; onWishlist: (id: string) => void }) {
  const [books, setBooks] = useState<Recommendation[]>([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => { gbSearchByAuthor(author).then(data => { setBooks(data || []); setLoading(false); }).catch(() => setLoading(false)); }, [author]);
  if (loading) return <div style={{ height: 200, background: 'rgba(255,255,255,0.03)', borderRadius: 6 }} />;
  if (!books.length) return null;
  return (<div><SectionHeader title={`More by ${author}`} count={books.length} /><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', paddingBottom: 8 }}>{books.map(book => <BookCard key={book.googleId} book={book} onAdd={() => onAdd(book)} inWishlist={wishlist.includes(book.googleId)} onWishlist={() => onWishlist(book.googleId)} />)}</div></div>);
}

// ── Book Card (Google Books) ─────────────────────────────────

function BookCard({ book, onAdd, inWishlist, onWishlist }: { book: Recommendation; onAdd: () => void; inWishlist: boolean; onWishlist: () => void }) {
  const [showDetail, setShowDetail] = useState(false);
  return (
    <>
