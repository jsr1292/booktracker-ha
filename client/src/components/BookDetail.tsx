    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e => e.stopPropagation()} style={{ maxWidth: 480, padding: 0, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontFamily: "'Libre Baskerville', serif", fontSize: 14, color: 'var(--text)' }}>Book Details</span>
        ...
        <div style={{ maxHeight: '70vh', overflowY: 'auto', padding: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: 16, marginBottom: 20 }}>
            {book.cover_url ? (
              ...
              <div style={{ width: 100, height: 150, background: 'var(--border)', borderRadius: 6, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 48 }}>📖</div>
            )}
            <div style={{ flex: 1, minWidth: 0 }}>
              <h2 style={{ fontFamily: "'Libre Baskerville', serif", fontSize: 16, color: 'var(--text)', marginBottom: 6, lineHeight: 1.3 }}>{book.title}</h2>
              <div style={{ fontSize: 12, color: 'var(--text2)', marginBottom: 10 }}>{book.author}</div>

              {/* Rating (user's) */}
              {book.rating && (
                <div style={{ fontSize: 16, color: 'var(--gold)', marginBottom: 8 }} className="stars">
              ...
              {olRatings && olRatings.average && (
                <div style={{ fontSize: 11, color: 'var(--gold)', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 9, color: 'var(--text3)', marginLeft: 4 }}>Open Library</span>
                </div>
              )}
          ...
          {book.description && (
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 9, color: 'var(--text2)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 8, fontFamily: "'JetBrains Mono', monospace" }}>
                Description
              </div>
              <div style={{ fontSize: 12, color: 'var(--text2)', lineHeight: 1.7, background: 'var(--border)', padding: '12px 14px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          {(authorWorksLoading || authorWorks.length > 0) && book.author && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 9, color: 'var(--text2)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 10, fontFamily: "'JetBrains Mono', monospace", display: 'flex', alignItems: 'center', gap: 6 }}>
...
function AuthorWorkCard({ work, onAdd }: { work: OLWork; onAdd: () => void }) {
  ...
  return (
    ...
    >
      <div style={{ width: '100%', height: 110, background: 'var(--border)', borderRadius: 5, position: 'relative', overflow: 'hidden' }}>
        {coverUrl ? (
...
function MetaItem({ label, value }: { label: string; value: string | null | undefined }) {
  ...
  return (
    <div style={{ background: 'var(--border)', borderRadius: 6, padding: '10px 12px', border: '1px solid rgba(255,255,255,0.04)' }}>
      <div style={{ fontSize: 9, color: 'var(--text2)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 4, fontFamily: "'JetBrains Mono', monospace" }}>{label}</div>
