...
export default function ReadingTimeline({ books }: Props) {
  ...
  if (!books.length) {
    return (
      <div className="glass-card" style={{ textAlign: 'center', padding: '40px 20px' }}>
        ...
        <p style={{ fontSize: 12, color: '#8096b4' }}>No reading history yet</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <div style={{ fontFamily: "'Libre Baskerville', serif", fontSize: 15, color: '#d4dce8' }}>Reading Timeline</div>
        <div style={{ fontSize: 10, color: '#8096b4', marginTop: 2 }}>{books.length} books across {grouped.size} months</div>
      </div>
      ...
      {undated.length > 0 && (
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            <div className="timeline-dot" style={{ background: '#6a7a8a' }} />
            <span style={{ fontSize: 10, fontWeight: 600, color: '#8096b4', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
              Undated
            </span>
              ...
              <div key={book.id} className="book-card" style={{ padding: '10px 14px' }}>
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    ...
                    {book.pages && (
                      <div style={{ fontSize: 10, color: '#6a7a8a', marginTop: 2 }}>{book.pages}p</div>
                    )}
                  </div>
                </div>
              </div>
