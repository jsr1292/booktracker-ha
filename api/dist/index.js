...
app.get('/api/stats', requireAuth, (req, res) => {
    try {
        ...
        const total = db.prepare('SELECT COUNT(*) as c FROM books WHERE user_id=?').get(userId).c;
        const finishedRows = db.prepare("SELECT COUNT(*) as c FROM books WHERE user_id=? AND status='finished'").get(userId);
        const finished = finishedRows.c;
        const reading = db.prepare("SELECT COUNT(*) as c FROM books WHERE user_id=? AND status='reading'").get(userId).c;
        const totalPagesRow = db.prepare("SELECT COALESCE(SUM(pages),0) as total FROM books WHERE user_id=? AND status='finished' AND pages > 0").get(userId);
        const totalPages = totalPagesRow.total;
        const avgPagesRow = db.prepare("SELECT COALESCE(AVG(pages),0) as avg FROM books WHERE user_id=? AND status='finished' AND pages > 0").get(userId);
        const avgPages = Math.round(avgPagesRow.avg);
        const avgRatingRow = db.prepare("SELECT COALESCE(AVG(rating),0) as avg FROM books WHERE user_id=? AND status='finished' AND rating IS NOT NULL").get(userId);
        const globalAvgRating = avgRatingRow.avg > 0 ? Math.round(avgRatingRow.avg * 10) / 10 : null;
        const ratedRows = db.prepare("SELECT COUNT(*) as c FROM books WHERE user_id=? AND status='finished' AND rating IS NOT NULL").get(userId);
    ...
    `).get(userId);
        ...
        // Genre distribution
        const genreRows = db.prepare("SELECT genre, COUNT(*) as count FROM books WHERE user_id=? AND status='finished' AND genre IS NOT NULL GROUP BY genre ORDER BY count DESC").all(userId);
        const genreDistribution = genreRows.map(g => ({ genre: g.genre, count: g.count }));
        const genreCount = genreDistribution.length;
        // Unique authors read
        const authorCount = db.prepare("SELECT COUNT(DISTINCT author) as c FROM books WHERE user_id=? AND status='finished' AND author IS NOT NULL AND author != ''").get(userId).c;
        // Longest book (pages)
        const longestBook = db.prepare("SELECT COALESCE(MAX(pages),0) as max FROM books WHERE user_id=? AND status='finished' AND pages > 0").get(userId).max;
        // Shortest book (pages)
        const shortestBook = db.prepare("SELECT COALESCE(MIN(pages),0) as min FROM books WHERE user_id=? AND status='finished' AND pages > 0").get(userId).min;
        // Highest rated book
        const topRated = db.prepare("SELECT COALESCE(MAX(rating),0) as max FROM books WHERE user_id=? AND status='finished' AND rating IS NOT NULL").get(userId).max;
        // Planned books count
        const planned = db.prepare("SELECT COUNT(*) as c FROM books WHERE user_id=? AND status='planned'").get(userId).c;
        // Abandoned count
        const abandoned = db.prepare("SELECT COUNT(*) as c FROM books WHERE user_id=? AND status='abandoned'").get(userId).c;
    ...
    `).get(userId).max;
        ...
        const highRatings = db.prepare("SELECT COUNT(*) as c FROM books WHERE user_id=? AND status='finished' AND rating >= 4").get(userId).c;
        ...
        for (const g of genreDistribution) {
            genreCounts[g.genre.toLowerCase()] = g.count;
        }
    ...
    `).all(userId);
        ...
        const ratingDist = [1, 2, 3, 4, 5].map(star => {
            const c = db.prepare("SELECT COUNT(*) as c FROM books WHERE user_id=? AND status='finished' AND rating=?").get(userId, star).c;
...
if (existsSync(newClientDist)) {
    const CSP = "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https://covers.openlibrary.org https://archive.org https://*.us.archive.org https://*.archive.org https://books.google.com https://books.google.com/books https://images-na.ssl-images-amazon.com; font-src 'self' https://fonts.gstatic.com; connect-src 'self' https://openlibrary.org https://www.googleapis.com; frame-ancestors 'none';";
